...
var DslConcreteStrategy = require('../../src/model//DslConcreteStrategy.js');
...
describe('DslConcreteStrategy TU-25', function() {
	...
	it('Ha un metodo loadDSLFile', function(done) {
		assert.notEqual(strategy.loadDSL, null);
		done();
	});
	...
	describe('Con un DSL semplice TU-27', function() {
		...
		it('Lo carica', function(done) {
			strategy.loadDSL(testdsl, domain, function(coll){
				collections = coll;
				done();
			}, function(err){
				done(err.toError());
	...
	describe('Con un DSL complesso TU-27', function() {
		// Errore nella formulazine della macro per quanto
		// riguarda i campi opzionali; non dovrebbe essere
		// necessario inserire le parentesi tonde.
		var testdsl = [
			"collection(name:'BBB') {",
			"	index () {",
